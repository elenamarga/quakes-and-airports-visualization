...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		int numberLandEarthquakes = 0;
		int numberOceanEarthquakes;
		
		for (int i = 0; i < countryMarkers.size(); i++) {		
			int numberEarthquakes = 0;
			Marker country = countryMarkers.get(i);
			
			for (int q = 0; q < quakeMarkers.size(); q++) {			
				Marker quake = quakeMarkers.get(q);
				
				if (country.getProperty("name") == quake.getProperty("country")) {
					numberEarthquakes++;
					numberLandEarthquakes++;
				}
			}
			
			if (numberEarthquakes > 0) {
				System.out.println(country.getProperty("name") + ": " + numberEarthquakes);
			}
		}
		
		numberOceanEarthquakes = quakeMarkers.size() - numberLandEarthquakes;
		System.out.println("OCEAN QUAKES: " + numberOceanEarthquakes);
	}
